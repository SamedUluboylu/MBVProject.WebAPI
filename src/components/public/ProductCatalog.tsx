...
const ProductCatalog: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center min-h-[60vh]">
        ...
        <div className="text-xl text-slate-600 font-medium">Loading amazing products...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <div className="bg-gradient-to-br from-white to-slate-50 rounded-2xl shadow-sm border border-slate-200 p-6 md:p-8">
        <h1 className="text-3xl md:text-4xl font-bold mb-2 text-slate-900">
          Discover Products
        </h1>
        <p className="text-slate-600 mb-6">Browse our collection of quality products</p>
        <div className="relative max-w-2xl">
          <input
            ...
            className="w-full px-6 py-4 pl-14 border-2 border-slate-200 rounded-xl focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all text-slate-900 placeholder-slate-400"
          />
      ...
      {products.length === 0 ? (
        ...
        <>
              ...
              >
                <div className="aspect-square bg-gradient-to-br from-slate-50 to-slate-100 relative overflow-hidden">
                  ...
                  {product.discountPercentage && (
                    <div className="absolute top-3 right-3 bg-gradient-to-r from-red-500 to-rose-600 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg z-10">
                ...
                <div className="p-5">
                  <h3 className="font-bold text-base mb-2 text-slate-900 line-clamp-2 group-hover:text-blue-600 transition-colors min-h-[3rem]">
                    {product.name}
                  </h3>
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
              <button
                onClick={() => setCurrentPage((p) => Math.max(1, p - 1))}
                disabled={currentPage === 1}
                className="px-5 py-2.5 border-2 border-slate-200 rounded-xl disabled:opacity-40 disabled:cursor-not-allowed hover:border-blue-500 hover:bg-blue-50 hover:text-blue-600 transition-all font-medium text-sm"
              >
                Previous
              </button>
